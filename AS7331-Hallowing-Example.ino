...
void setup(void) {
  ...
  if(myUVSensor.begin() == false) {
    Serial.println("AS7331 UV Sensor failed to begin. Please check your wiring!");
    Serial.println("Halting...");
    while(1);
  }

  Serial.println("AS7331 began.");

  // Set measurement mode and change device operating mode to measure.
  if(myUVSensor.prepareMeasurement(MEAS_MODE_CMD) == false) {
    Serial.println("AS7331 did not get set properly.");
    Serial.println("Halting...");
    while(1);
  }
  Serial.println("Set AS7331 mode to command.");
  /**** END AS7331 Setup ****/
}
...
void loop() {
  // arcada.display->fillRect(0, 0, 128, 128, ARCADA_BLACK);
  ...
  /* UV Sensor Readout */
  arcada.display->fillRect(0, 50, 128, 30, ARCADA_BLACK);
  ...
  arcada.display->print("UVA: ");
  arcada.display->println(myUVSensor.getUVA());
  arcada.display->print("UVB: ");
  arcada.display->println(myUVSensor.getUVB());
  arcada.display->print("UVC: ");
  arcada.display->println(myUVSensor.getUVC());
  /* END UV Sensor Readout */
